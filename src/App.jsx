...
function PaywallPage({ onBack, firstAnalysisDone }) {
  ...
  const checkout = async (priceKey) => {
    if (!email.trim()) { setError("Please enter your email address first."); return; }
    track("checkout_started", { plan: priceKey });
...
function LandingPage({ onStart, onMyStudio, onEmailVerified }) {
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", background: BG, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "2.5rem 2.5rem 3rem", boxSizing: "border-box", position: "relative", overflow: "hidden" }}>
      <TeknonLogo size="md" />
      <div style={{ maxWidth: 560, position: "relative", zIndex: 1 }}>
        ...
        {!showEmail ? (
          <div>
            <PillBtn onClick={() => { track("landing_get_started_clicked"); onStart(); }}>try for free</PillBtn>
...
const PORTRAIT_ARTISTS = [
  ...
  { file: "Merian.jpg",     name: "Maria Sibylla Merian" },
  ...
  { file: "Homer.jpg",      name: "Winslow Homer" },
            ...
            {entries.map((e, i) => (
              <div key={e.id}>
                <div style={{ marginBottom: "2rem" }}>
                  <p style={{ ...T.body, fontSize: "0.85rem", letterSpacing: "0.15em", color: T.amber, textTransform: "uppercase", marginBottom: "0.75rem" }}>
  ...
  useEffect(() => {
    if (!resources && targetArtist) {
      (async () => {
        ...
        try {
          const text = await callAPI([{ role: "user", content: `Find 4-6 real online resources that would genuinely help an artist with the specific issues raised in this feedback:\n\n${feedback}\n\nPrioritise practical tutorials, technique videos, and instructional articles that directly address the problems discussed. Only include resources about ${targetArtist} if they are specifically instructional about technique. Avoid biographical articles, museum pages, or general overviews.\n\nIMPORTANT: Only suggest resources that are freely accessible without a subscription or paywall. Prefer YouTube videos, free blog posts, open access museum resources, and free tutorial sites. Do not suggest articles from newspapers, magazines or sites that typically require payment such as The Art Newspaper, The Guardian arts section, or similar. Prefer sources like Jackson's Art Blog, Winsor & Newton tutorials, YouTube art channels, and open access museum learning resources.\n\nReturn ONLY valid JSON:\n[{"title":"...","url":"...","source":"...","type":"article|video|interview|tutorial|website","description":"one sentence"}]\n\nOnly real URLs.` }]);
          const c = text.replace(/```json|```/g, "").trim(); const s = c.indexOf("["), e = c.lastIndexOf("]");
